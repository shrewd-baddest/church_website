...
export default function MemberDashboard() {
  ...
  const chartData = weeks.map((w) => ({
    name: `W${w.week}`,
    fullName: `Week ${w.week}`,
    accuracy: w.totalAttempts
      ...
      <div className="max-w-5xl mx-auto px-6 pt-12 sm:pt-20 relative z-10">
        <div className="mb-12 animate-in fade-in slide-in-from-top-6 duration-700">
          ...
          <p className="text-slate-500 font-medium mt-3 max-w-xl leading-relaxed">
            Track your milestones, accuracy, and attempts. Data is refreshed when admin publishes updates.
          </p>
        </div>
        ...
        {summary ? (
            ...
            <div className="bg-white/80 backdrop-blur-xl rounded-[2.5rem] p-8 md:p-10 border border-white shadow-[0_8px_40px_rgb(0,0,0,0.04)] animate-in fade-in slide-in-from-bottom-10 duration-700 delay-200">
              <div className="flex flex-col sm:flex-row items-center justify-between mb-10 gap-4">
                <div>
                  ...
                  <p className="text-xs font-bold text-slate-400 mt-1">Accuracy trends over published weeks</p>
                </div>
                <div className="px-4 py-2 bg-blue-50 rounded-xl border border-blue-100 flex items-center gap-2">
                  <span className="w-2 h-2 rounded-full bg-blue-500 animate-pulse"></span>
                  <span className="text-[10px] font-black uppercase tracking-widest text-blue-600">Published Data</span>
                </div>
              </div>
              ...
              {chartData.length > 0 ? (
                <div className="h-[350px] w-full mt-4">
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={chartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                      ...
                      <Tooltip
                        contentStyle={{ borderRadius: '16px', border: 'none', boxShadow: '0 10px 25px -5px rgba(0, 0, 0, 0.1)', padding: '12px 20px', fontWeight: 700 }}
                      ...
                      />
                      <Area type="monotone" dataKey="accuracy" stroke="#2563eb" strokeWidth={4} fillOpacity={1} fill="url(#colorAccuracy)" />
                    </AreaChart>
                  </ResponsiveContainer>
                </div>
              ) : (
                <div className="flex flex-col items-center justify-center py-20 text-center bg-slate-50 rounded-3xl border border-slate-100 border-dashed">
                  ...
                  <h3 className="text-lg font-black text-slate-800">No Published Data Yet</h3>
                  <p className="text-sm font-medium text-slate-500 max-w-sm mt-2">
                    Admin has not published progress data yet. Check back after the next publish.
                  </p>
                </div>
              )}
            </div>
          ...
          <div className="flex flex-col items-center justify-center py-32 text-center bg-white rounded-[3rem] border border-slate-100 shadow-sm animate-in fade-in zoom-in duration-700">
            ...
            <h3 className="text-2xl font-black text-slate-900">No Published Progress</h3>
            <p className="text-slate-500 font-medium mt-3 max-w-sm">
              Wait for an admin to publish the latest stats.
            </p>
          </div>
        )}
      </div>
